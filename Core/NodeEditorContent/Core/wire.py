...
class Wire(QGraphicsPathItem):
    ...
    def _set_pen(self, selected: bool):
        ...
        self.setPen(QPen(c.lighter(160) if selected else c,
...
class TempWire(QGraphicsPathItem):
    def __init__(self, src_pin):
        ...
        self.setPen(QPen(pin_color(src_pin.pin_type), 2, Qt.DashLine, Qt.RoundCap))
        self.setZValue(100)
    ...
    def update_end(self, end):
        ...
        path = QPainterPath(p1)
        path.cubicTo(p1.x() + dx, p1.y(), end.x() - dx, end.y(), end.x(), end.y())
        self.setPath(path)