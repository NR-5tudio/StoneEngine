"""pin.py  –  Pin with inline value widgets"""
...
def _make_widget(ptype: str, saved_value):
    ...
    if ptype == 'float':
        w = QDoubleSpinBox()
        w.setRange(-1e9, 1e9); w.setDecimals(3); w.setSingleStep(0.1)
        w.setButtonSymbols(QDoubleSpinBox.NoButtons)
        w.setFixedSize(64, 18); w.setStyleSheet(style)
        try: w.setValue(float(saved_value))
    ...
    if ptype == 'int':
        ...
        w.setFixedSize(56, 18); w.setStyleSheet(style)
        try: w.setValue(int(saved_value))
        except: w.setValue(0)
        return w
    if ptype == 'bool':
        w = QCheckBox()
        ...
        try: w.setChecked(bool(saved_value))
        except: w.setChecked(False)
        return w
    if ptype == 'string':
        w = QLineEdit()
        w.setFixedSize(72, 18); w.setStyleSheet(style)
        w.setText(str(saved_value) if saved_value is not None else "")
        return w
    if ptype == 'custom':
        w = QLineEdit()
        w.setFixedSize(72, 18); w.setStyleSheet(style)
        w.setPlaceholderText("C++ expr")
        w.setText(str(saved_value) if saved_value is not None else "")
        return w
    return None
...
def _default_value(ptype: str):
    return {'float': 0.0, 'int': 0, 'bool': False, 'string': '', 'custom': ''}.get(ptype, 0.0)
...
class Pin(QGraphicsEllipseItem):
    def __init__(self, node, name: str, pin_type: str, direction: str, index: int):
        ...
        self.index      = index
        self.wires      = []
        self._saved_val = _default_value(pin_type)
    ...
    def _place_children(self):
        ...
        if self.direction == 'in':
            ...
            if self._proxy:
                self._proxy.setPos(PIN_R + 4 + lw + 4, -9)
        else:
    ...
    def on_disconnected(self):
        if self._proxy:
            if self._widget:
                if self.pin_type == 'float':   self._widget.setValue(float(self._saved_val))
                elif self.pin_type == 'int':   self._widget.setValue(int(self._saved_val))
                elif self.pin_type == 'bool':  self._widget.setChecked(bool(self._saved_val))
                else:                          self._widget.setText(str(self._saved_val))
            self._proxy.show()
    ...
    def paint(self, painter, option, widget=None):
        ...
        if self.pin_type == 'float':
            path = QPainterPath()
            path.moveTo(0, -PIN_R); path.lineTo(PIN_R, 0)
            path.lineTo(0, PIN_R);  path.lineTo(-PIN_R, 0)
            path.closeSubpath()
    ...
    def hoverEnterEvent(self, e):
        self.setPen(QPen(self._color.lighter(160), 2)); self.update()
        super().hoverEnterEvent(e)

    def hoverLeaveEvent(self, e):
        self.setPen(QPen(self._color.darker(150), 1)); self.update()
